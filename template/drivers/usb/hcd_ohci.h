...
/* NON-BLOCKING presence test (hot-plug): returns 1 if a device is currently
 * connected on one of the root-hub ports (HcRhPortStatus.CCS), 0 otherwise (or
 * if the controller is not initialized). Reads only, never resets, never logs:
 * meant to be polled in a permanent loop (kbd_service). */

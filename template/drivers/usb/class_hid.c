...
/* ------------------------------------------------------------------ */
/* Character sink (production: routed to the shell line editor)        */
/* ------------------------------------------------------------------ */
static hid_kbd_sink_fn s_sink;
...
int hid_kbd_poll(hid_kbd_t *kbd, uint32_t timeout_ms)
{
    ...
    for (int i = 2; i < 8; i++) {
        ...
        if (key < 0x64) {
            /* Mapped character -> sink (shell). Unmapped keycode: SILENTLY
             * ignored (production console stays clean; no periodic noise). */
